...
GLint (*faces)[3];     // Face indices
GLfloat (*normals)[3]; // Face normals
int face_count;        // Face count
GLfloat (*v)[3];       // Vertex positions
...
void pushToGL(Forth::Buffer3 out)
{
	...
	size_t vlen = out.vertices.size();
	size_t ilen = out.indices.size();
	v = new float[vlen][3];
	...
	for (int i = 0; i < face_count; i++)
	{
		Forth::Vector3 a = Forth::Vector3(v[faces[i][0]]),
					   ...
					   cross = Forth::Normalize(Forth::Cross(a - c, b - c));
